...
struct MiraWaveform: View {
    ...
    private var totalWidth: CGFloat {
        let bars = Self.barWidth * size.scale * 5
        let gaps = Self.barGap * size.scale * 5
        let starW = Self.starSize * size.scale + 2 * size.scale
        return bars + gaps + starW
    }

    private var totalHeight: CGFloat {
        Self.barHeights[2] * size.scale * 1.4
    }
